...
function Login({ setIsLogin }: Props) {
    ...
    const onSubmit = handleSubmit(async (values) => {
        try {
            const { data: { id, email, token } } = await axios.post<UserType>(`${process.env.REACT_APP_API_URL}/auth/signin`, { email: values.email, password: values.password })
    ...
    return (
        <>
            ...
            <form onSubmit={onSubmit}>
                ...
                <button className='text-sm text-sky-700 mb-4' onClick={() => setIsLogin(false)}>New User? Sign Up </button>
