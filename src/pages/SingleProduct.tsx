...
function SingleProduct() {
    ...
    const onSubmit = handleSubmit(async (values) => {
        try {
            await axios.patch(`${process.env.REACT_APP_API_URL}/products/${id}`, { title: values.title, description: values.description, price: values.price })
            navigate("/")
        } catch (error) {
            console.log(error)
            alert("ERROR")
    ...
    useEffect(() => {
        const populateValues = async () => {
            try {
                const { data: { title, description, price } } = await axios.get<ProductType>(`${process.env.REACT_APP_API_URL}/products/${id}`, { headers: { Authorization: `Bearer ${user?.token}` } })
